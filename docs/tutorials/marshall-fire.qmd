...
We use [Landfire.jl](https://github.com/RallypointOne/Landfire.jl) to download 30 m FBFM13 fuel models and elevation for the fire area.

```{julia}
#| code-summary: "Download LANDFIRE FBFM13 and elevation"

# Bounding box covering the Marshall Fire (with buffer)
aoi = "-105.238 39.924 -105.126 39.992"
...
for i in eachindex(fuel_ids)
    ...
    elseif v >= 98
        fuel_ids[i] = 256  # water/bare → nonburnable
    end
end
...
# Ignition point from NIFC metadata
ign_lon = feat.properties.attr_InitialLongitude
ign_lat = feat.properties.attr_InitialLatitude
...
println("Ignition:  col=$ign_col, row=$ign_row ($(round(ign_lon, digits=4)), $(round(ign_lat, digits=4)))")
```
...
The effective wind speed (average of sustained and gust) captures the influence of gusts
on fire spread better than sustained speed alone.
...
- **LANDFIRE FBFM13 & Elevation**: [LANDFIRE Product Service](https://lfps.usgs.gov) v2.5.0, downloaded via [Landfire.jl](https://github.com/RallypointOne/Landfire.jl)
