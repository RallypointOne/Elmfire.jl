...
Aspect is the direction the slope *faces*, so fire runs upslope — toward
`aspect - 180`. That means aspect decides whether terrain reinforces the wind or
fights it. With wind from the west (blowing east):

- **West-facing** (upslope points east): slope and wind agree, largest fire
- **East-facing** (upslope points west): slope opposes the wind and the two partly
  cancel, smallest fire
- **North-** and **south-facing**: upslope is perpendicular to the wind, so both
  give the same area, differing only in which way the fire is pushed off-axis

```{julia}
#| code-summary: "Fire spread on a 15° slope at four aspects"
...
for (asp, lbl) in zip(aspects, aspect_labels)
    state = FireState{Float64}(70, 70, 30.0)
    weather = ConstantWeather{Float64}(
        wind_speed_mph = 5.0,
    ...
    ignite!(state, 35, 35, 0.0)
    simulate_uniform!(state, 1, fuel_table, weather, 15.0, Float64(asp), 0.0, 60.0)
...
plot(plots..., layout = (2, 2), size = (700, 720),
    plot_title = "15° Slope with Different Aspects (Wind from West)")
```

The west-facing slope burns roughly seven times the area of the east-facing one
under identical wind and fuel — terrain aiding the wind versus opposing it. North-
and south-facing come out identical, as the symmetry requires.
...
Fire behavior changes dramatically at terrain features. Igniting west of a N-S
ridge, with wind from the west, puts slope and wind in agreement on the windward
flank and in opposition on the lee side:
...
for ix in 1:ncols
    for iy in 1:nrows
        ...
        if dist < 30
            ...
            # Aspect is the downhill direction. West of the crest the ground rises
            # toward the east, so it faces west (270°); east of the crest it faces
            # east (90°).
            aspect[ix, iy] = ix < 60 ? 270.0 : 90.0
        end
    end
end
...
simulate!(state, fuel_ids, fuel_table, weather, slope, aspect, 0.0, 90.0)
...
vline!(p2, [60], color = :black, linestyle = :dash, linewidth = 2, label = "")

plot(p1, p2, layout = (1, 2), size = (800, 400))
```

The fire runs quickly up the windward flank, where the upslope direction and the
wind reinforce each other, then slows sharply once it crosses the crest (dashed
line) and the slope begins to oppose the wind. At 10 mph it does not cross at all —
it stalls at the crest.
